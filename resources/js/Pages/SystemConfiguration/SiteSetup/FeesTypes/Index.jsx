...
export default function Index({ auth, feestypes, filters }) {
    ...
    const [modalState, setModalState] = useState({
        ...
        message: '',
        isAlert: false,
        feestypeToDeleteId: null,
    });

    useEffect(() => {
        get(route("systemconfiguration0.feestypes.index"), { preserveState: true });
    ...
    const handleDelete = (id) => {
        setModalState({
            isOpen: true,
            message: "Are you sure you want to delete this feestype?",
            isAlert: false,
            feestypeToDeleteId: id,
        });
    };

    const handleModalClose = () => {
        setModalState({ isOpen: false, message: '', isAlert: false, feestypeToDeleteId: null });
    };

    const handleModalConfirm = async () => {
        try {
            await router.delete(route("systemconfiguration0.feestypes.destroy", modalState.feestypeToDeleteId));
        } catch (error) {
            console.error("Failed to delete feestype:", error);
            showAlert("There was an error deleting the feestype. Please try again.");
        }
        setModalState({ isOpen: false, message: '', isAlert: false, feestypeToDeleteId: null });
    };
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold text-gray-800">Fees List</h2>}
        >
            <Head title="Fees List" />
            <div className="container mx-auto p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <div className="flex items-center space-x-2 mb-4 md:mb-0">
                        ...
                        <Link
                            href={route("systemconfiguration0.feestypes.create")}
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full border border-gray-300 shadow-md rounded">
                        ...
                        <tbody>
                            {feestypes.data.length > 0 ? (
                                feestypes.data.map((feestype, index) => (
                                    <tr key={feestype.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                                        <td className="border-b p-3 text-gray-700">{feestype.name ? feestype.name : "n/a"}</td>  
                                        <td className="border-b p-3 text-right text-gray-700">                                           
                                            {parseFloat(feestype.amount).toLocaleString(undefined, {
                                        ...
                                        <td className="border-b p-3 flex space-x-2">
                                            <Link
                                                href={route("systemconfiguration0.feestypes.edit", feestype.id)}
