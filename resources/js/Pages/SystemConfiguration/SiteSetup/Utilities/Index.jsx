...
export default function Index({ auth, utilities, filters }) {
    ...
    const [modalState, setModalState] = useState({
        ...
        message: '',
        isAlert: false,
        utilityToDeleteId: null,
    });

    useEffect(() => {
        get(route("systemconfiguration0.utilities.index"), { preserveState: true });
    ...
    const handleDelete = (id) => {
        setModalState({
            ...
            isAlert: false,
            utilityToDeleteId: id,
        });
    };

    const handleModalClose = () => {
        setModalState({ isOpen: false, message: '', isAlert: false, utilityToDeleteId: null });
    };

    const handleModalConfirm = async () => {
        try {
            await router.delete(route("systemconfiguration0.utilities.destroy", modalState.utilityToDeleteId));
        } catch (error) {
            console.error("Failed to delete utility:", error);
            showAlert("There was an error deleting the utility. Please try again.");
        }
        setModalState({ isOpen: false, message: '', isAlert: false, utilityToDeleteId: null });
    };
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold text-gray-800">Utilities List</h2>}
        >
            <Head title="Ulitities List" />
            <div className="container mx-auto p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <div className="flex items-center space-x-2 mb-4 md:mb-0">
                        ...
                        <Link
                            href={route("systemconfiguration0.utilities.create")}
                        ...
                        <tbody>
                            {utilities.data.length > 0 ? (
                                utilities.data.map((utility, index) => (
                                    <tr key={utility.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                                        <td className="border-b p-3 text-gray-700">{utility.name ? utility.name : "n/a"}</td> 
                                        <td className="border-b p-3 flex space-x-2">
                                            <Link
                                                href={route("systemconfiguration0.utilities.edit", utility.id)}
