...
export default function Index({ auth, activities, filters }) {
    ...
    useEffect(() => {
        get(route("systemconfiguration0.activities.index"), { preserveState: true });
    ...
    const handleModalClose = () => {
        setModalState({ isOpen: false, message: '', isAlert: false, activityToDeleteId: null });
    };

    const handleModalConfirm = async () => {
        try {
            await router.delete(route("systemconfiguration0.activities.destroy", modalState.activityToDeleteId));
        } catch (error) {
            console.error("Failed to delete activity:", error);
            showAlert("There was an error deleting the activity. Please try again.");
        }
        setModalState({ isOpen: false, message: '', isAlert: false, activityToDeleteId: null });
    };
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold text-gray-800">Activities List</h2>}
        >
            <Head title="Activities List" />
            <div className="container mx-auto p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <div className="flex items-center space-x-2 mb-4 md:mb-0">
                        ...
                        <Link
                            href={route("systemconfiguration0.activities.create")}
                        ...
                        <tbody>
                            {activities.data.length > 0 ? (
                                activities.data.map((activity, index) => (
                                    <tr key={activity.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                                        <td className="border-b p-3 text-gray-700">{activity.name ? activity.name : "n/a"}</td>                                           
                                        <td className="border-b p-3 flex space-x-2">
                                            <Link
                                                href={route("systemconfiguration0.activities.edit", activity.id)}
                                ...
                                <tr>
                                    <td colSpan="4" className="border-b p-3 text-center text-gray-700">No Activity found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
