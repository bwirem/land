...
export default function Index({ auth, loanpackages, filters }) {
    ...
    const [modalState, setModalState] = useState({
        ...
        message: '',
        isAlert: false,
        loanpackageToDeleteId: null,
    });

    useEffect(() => {
        get(route("systemconfiguration0.loanpackages.index"), { preserveState: true });
    }, [data.search, get]);
    ...
    const handleDelete = (id) => {
        setModalState({
            ...
            isAlert: false,
            loanpackageToDeleteId: id,
        });
    };

    const handleModalClose = () => {
        setModalState({ isOpen: false, message: '', isAlert: false, loanpackageToDeleteId: null });
    };

    const handleModalConfirm = async () => {
        try {
            await router.delete(route("systemconfiguration0.loanpackages.destroy", modalState.loanpackageToDeleteId));
        } catch (error) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold text-gray-800">Package List</h2>}
        >
            <Head title="Package List" />
            <div className="container mx-auto p-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                    <div className="flex items-center space-x-2 mb-4 md:mb-0">
                        ...
                        <Link
                            href={route("systemconfiguration0.loanpackages.create")}
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full border border-gray-300 shadow-md rounded">
                        ...
                        <tbody>
                            {loanpackages.data.length > 0 ? (
                                loanpackages.data.map((loanpackage, index) => (
                                    <tr key={loanpackage.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                                        ...
                                        <td className="border-b p-3 flex space-x-2 justify-center">
                                            <Link
                                                href={route("systemconfiguration0.loanpackages.edit", loanpackage.id)}
                                            ...
                                            <button
                                                onClick={() => handleDelete(loanpackage.id)}
