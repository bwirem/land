...
export default function Documentation({ site, siteTypes }) {
    ...
    const { data, setData, put, errors, processing, reset } = useForm({
        ...
        siteAmount: site.site_amount,
        siteDuration: site.site_duration,
        interestRate: site.interest_rate,
        interestAmount: site.interest_amount,
        monthlyRepayment: site.monthly_repayment,
        totalRepayment: site.total_repayment,
        stage: site.stage,
        guarantors: site.site_guarantors || [],  // Array of guarantor details
    });
    ...
    useEffect(() => {
        ...
        setExistingGuarantorIds(site.site_guarantors.map(guarantor => guarantor.guarantor_id));
    }, [site.site_guarantors]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(route('landowner1.documentation', site.id), formData, {
                headers: {
   ...
   const handleSubmitModalConfirm = () => {
       ...
       axios.post(route('landowner1.submit', site.id), submitData) // Assuming you create a new route
           .then(response => {
               ...
               if (response.status === 200) { // Check the status code for success
                   Inertia.get(route('landowner1.index')); // Navigate to procurements0.index
               } else {
           ...
           .catch(error => {
               console.error("Error Submiting Site:", error);
    ...
    >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-gray-500">
                                        {siteTypes.find(type => type.id === site.site_type)?.name || 'N/A'}
                                    </p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Site Amount:</label>
                                    <p className="mt-1 text-sm text-gray-500">
                                        {parseFloat(data.siteAmount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Tsh
                                    </p>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Site Duration:</label>
                                    <p className="mt-1 text-sm text-gray-500">{data.siteDuration} Months</p>
                                </div>
                                <div>
        ...
        <Modal
            ...
            >
                <div>
                    <p>
                        Are you sure you want to submit the site to <strong>
                            {data.customer_type === 'individual' ? (
